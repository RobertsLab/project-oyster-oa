...
for (j in 4:12) { #For each column with pH data
  for(i in 1:nDates) { #For each date
    pHMinimum[i, j-2] <- min(pHTideData[pHTideData$Date == outplantDates[i], j], na.rm = TRUE) #Find the minimum value of the designated column at each date. The minimum value will be entered in the specified column of the dataframe pHMinimum.
  }
...
is.na(pHMinimum) <- sapply(pHMinimum, is.infinite) #Replace all "Inf" with N/A
head(pHMinimum) #Confirm dataframe filling. All "Inf" values are now replaced
```
...
```{r}
pHMaximum <- data.frame("Date" = outplantDates,
                        "WBE-pH-max" = rep(0, length(outplantDates)),
                        "WBB-pH-max" = rep(0, length(outplantDates)),
                        "SKE-pH-max" = rep(0, length(outplantDates)),
                        "SKB-pH-max" = rep(0, length(outplantDates)),
                        "PGB-pH-max" = rep(0, length(outplantDates)),
                        "CIE-pH-max" = rep(0, length(outplantDates)),
                        "CIB-pH-max" = rep(0, length(outplantDates)),
                        "FBE-pH-max" = rep(0, length(outplantDates)),
                        "FBB-pH-max" = rep(0, length(outplantDates))) #Create an empty dataframe
head(pHMaximum)
```
...
for (j in 4:12) { #For each column with pH data
  for(i in 1:nDates) { #For each date
    pHMaximum[i, j-2] <- max(pHTideData[pHTideData$Date == outplantDates[i], j], na.rm = TRUE) #Find and save the maximum value of the designated column at each date.
  }
} #The loop will cycle through each date first in one column, then move to the next column. Any days with no non-missing data will save as "Inf" in the dataframe.
is.na(pHMaximum) <- sapply(pHMaximum, is.infinite) #Replace all "Inf" with N/A
head(pHMaximum) #Confirm dataframe filling. All "Inf" values are now replaced
```
...
```{r}
DOMinimum <- data.frame("Date" = outplantDates,
                        "WBE-DO-min" = rep(0, length(outplantDates)),
                        "WBB-DO-min" = rep(0, length(outplantDates)),
                        "SKE-DP-min" = rep(0, length(outplantDates)),
                        "SKB-DO-min" = rep(0, length(outplantDates)),
                        "PGE-DO-min" = rep(0, length(outplantDates)),
                        "PGB-DO-min" = rep(0, length(outplantDates)),
                        "CIE-DO-min" = rep(0, length(outplantDates)),
                        "CIB-DO-min" = rep(0, length(outplantDates)),
                        "FBE-DO-min" = rep(0, length(outplantDates)),
                        "FBB-DO-min"  = rep(0, length(outplantDates))) #Create an empty dataframe
head(DOMinimum)
```
...
for (j in 4:13) { #For each column with DO data
  for(i in 1:nDates) { #For each date
    DOMinimum[i, j-2] <- min(DOTideData[DOTideData$Date == outplantDates[i], j], na.rm = TRUE) #Find and save the minimum value of the designated column at each date.
  }
} #The loop will cycle through each date first in one column, then move to the next column. Any days with no non-missing data will save as "Inf" in the dataframe.
is.na(DOMinimum) <- sapply(DOMinimum, is.infinite) #Replace all "Inf" with N/A
head(DOMinimum) #Confirm dataframe filling. All "Inf" values are now replaced
```
...
```{r}
DOMaximum <- data.frame("Date" = outplantDates,
                        "WBE-DO-max" = rep(0, length(outplantDates)),
                        "WBB-DO-max" = rep(0, length(outplantDates)),
                        "SKE-DP-max" = rep(0, length(outplantDates)),
                        "SKB-DO-max" = rep(0, length(outplantDates)),
                        "PGE-DO-max" = rep(0, length(outplantDates)),
                        "PGB-DO-max" = rep(0, length(outplantDates)),
                        "CIE-DO-max" = rep(0, length(outplantDates)),
                        "CIB-DO-max" = rep(0, length(outplantDates)),
                        "FBE-DO-max" = rep(0, length(outplantDates)),
                        "FBB-DO-max" = rep(0, length(outplantDates))) #Create an empty dataframe
head(DOMaximum)
```
...
for (j in 4:13) { #For each column with DO data
  for(i in 1:nDates) { #For each date
    DOMaximum[i, j-2] <- min(DOTideData[DOTideData$Date == outplantDates[i], j], na.rm = TRUE) #Find and save the maximum value of the designated column at each date.
  }
} #The loop will cycle through each date first in one column, then move to the next column. Any days with no non-missing data will save as "Inf" in the dataframe.
is.na(DOMaximum) <- sapply(DOMaximum, is.infinite) #Replace all "Inf" with N/A
head(DOMaximum) #Confirm dataframe filling. All "Inf" values are now replaced
```
...
```{r}
salinityMinimum <- data.frame("Date" = outplantDates,
                              "CIB-sal-min" = rep(0, length(outplantDates)),
                              "CIE-sal-min" = rep(0, length(outplantDates)),
                              "FBB-sal-min" = rep(0, length(outplantDates)),
                              "FBE-sal-min" = rep(0, length(outplantDates)),
                              "PGE-sal-min" = rep(0, length(outplantDates)),
                              "SKB-sal-min" = rep(0, length(outplantDates)),
                              "SKE-sal-min" = rep(0, length(outplantDates)),
                              "WBB-sal-min" = rep(0, length(outplantDates))) #Create an empty dataframe
head(salinityMinimum)
```

```{r}
for (j in 4:11) { #For each column with salinity data
  for(i in 1:nDates) { #For each date
    salinityMinimum[i, j-2] <- min(salinityTideData[salinityTideData$Date == outplantDates[i], j], na.rm = TRUE) #Find and save the minimum value of the designated column at each date.
  }
} #The loop will cycle through each date first in one column, then move to the next column. Any days with no non-missing data will save as "Inf" in the dataframe.
is.na(salinityMinimum) <- sapply(salinityMinimum, is.infinite) #Replace all "Inf" with N/A
head(salinityMinimum) #Confirm dataframe filling. All "Inf" values are now replaced
```
...
```{r}
salinityMaximum <- data.frame("Date" = outplantDates,
                              "CIB-sal-max" = rep(0, length(outplantDates)),
                              "CIE-sal-max" = rep(0, length(outplantDates)),
                              "FBB-sal-max" = rep(0, length(outplantDates)),
                              "FBE-sal-max" = rep(0, length(outplantDates)),
                              "PGE-sal-max" = rep(0, length(outplantDates)),
                              "SKB-sal-max" = rep(0, length(outplantDates)),
                              "SKE-sal-max" = rep(0, length(outplantDates)),
                              "WBB-sal-max" = rep(0, length(outplantDates))) #Create an empty dataframe
head(salinityMaximum)
```

```{r}
for (j in 4:11) { #For each column with salinity data
  for(i in 1:nDates) { #For each date
    salinityMaximum[i, j-2] <- max(salinityTideData[salinityTideData$Date == outplantDates[i], j], na.rm = TRUE) #Find and save the maximum value of the designated column at each date.
  }
} #The loop will cycle through each date first in one column, then move to the next column. Any days with no non-missing data will save as "Inf" in the dataframe.
is.na(salinityMaximum) <- sapply(salinityMaximum, is.infinite) #Replace all "Inf" with N/A
head(salinityMaximum) #Confirm dataframe filling. All "Inf" values are now replaced
```
...
```{r}
temperatureMinimum <- data.frame("Date" = outplantDateCharacters,
                                 "WBE-temp-min" = rep(0, length(outplantDates)),
                                 "WBB-temp-min" = rep(0, length(outplantDates)),
                                 "SKE-temp-min" = rep(0, length(outplantDates)),
                                 "SKB-temp-min" = rep(0, length(outplantDates)),
                                 "PGE-temp-min" = rep(0, length(outplantDates)),
                                 "PGB-temp-min" = rep(0, length(outplantDates)),
                                 "CIE-temp-min" = rep(0, length(outplantDates)),
                                 "CIB-temp-min" = rep(0, length(outplantDates)),
                                 "FBE-temp-min" = rep(0, length(outplantDates)),
                                 "FBB-temp-min" = rep(0, length(outplantDates))) #Create an empty dataframe, but be sure to use outplantDateCharacters
head(temperatureMinimum)
```

```{r}
for (j in 3:12) { #For each column with temperature data
  for(i in 1:nDates) { #For each date
    temperatureMinimum[i, j-1] <- min(temperatureData[temperatureData$DateCharacters == outplantDateCharacters[i], j], na.rm = TRUE) #Find and save the minimum value of the designated column at each date.
  }
} #The loop will cycle through each date first in one column, then move to the next column. Any days with no non-missing data will save as "Inf" in the dataframe.
is.na(temperatureMinimum) <- sapply(temperatureMinimum, is.infinite) #Replace all "Inf" with N/A
head(temperatureMinimum) #Confirm dataframe filling. All "Inf" values are now replaced
```

```{r}
temperatureMinimum$Date <- outplantDates #Replace temperatureMinimum$Date with outplantDates so they are dates 
temperatureMinimum$FBB.temp.min <- as.numeric(temperatureMinimum$FBB.temp.min) #Convert characters in to numeric values
head(temperatureMinimum)
```
...
```{r}
temperatureMaximum <- data.frame("Date" = outplantDateCharacters,
                                 "WBE-temp-max" = rep(0, length(outplantDates)),
                                 "WBB-temp-max" = rep(0, length(outplantDates)),
                                 "SKE-temp-max" = rep(0, length(outplantDates)),
                                 "SKB-temp-max" = rep(0, length(outplantDates)),
                                 "PGE-temp-max" = rep(0, length(outplantDates)),
                                 "PGB-temp-max" = rep(0, length(outplantDates)),
                                 "CIE-temp-max" = rep(0, length(outplantDates)),
                                 "CIB-temp-max" = rep(0, length(outplantDates)),
                                 "FBE-temp-max" = rep(0, length(outplantDates)),
                                 "FBB-temp-max" = rep(0, length(outplantDates))) #Create an empty dataframe, but be sure to use outplantDateCharacters
head(temperatureMaximum)
```

```{r}
for (j in 3:12) { #For each column with temperature data
  for(i in 1:nDates) { #For each date
    temperatureMaximum[i, j-1] <- max(temperatureData[temperatureData$DateCharacters == outplantDateCharacters[i], j], na.rm = TRUE) #Find and save the minimum value of the designated column at each date.
  }
} #The loop will cycle through each date first in one column, then move to the next column. Any days with no non-missing data will save as "Inf" in the dataframe.
is.na(temperatureMaximum) <- sapply(temperatureMaximum, is.infinite) #Replace all "Inf" with N/A
head(temperatureMaximum) #Confirm dataframe filling. All "Inf" values are now replaced
```

```{r}
temperatureMaximum$Date <- outplantDates #Replace temperatureMaximum$Date with outplantDates so they are dates
head(temperatureMaximum)
```
...
```{r}
dailyEnvironmentalData <- cbind(pHMinimum, pHMean, pHVariance, pHMaximum,
                                DOMinimum, DOMean, DOVariance, DOMaximum,
                                salinityMinimum, salinityMean, salinityVariance, salinityMaximum,
                                temperatureMinimum, temperatureMean, temperatureVariance, temperatureMaximum) #Use cbind to merge all of the dataframes together
colnames(dailyEnvironmentalData) #View column names
```

```{r}
dailyEnvironmentalData <- dailyEnvironmentalData[-c(11, 21, 31, 41, 52, 63, 74, 85, 94, 103, 112, 121, 132, 143, 154)] #Remove all redundant "Date" columns
...
#dailyEnvironmentalDataCorrected[is.na(dailyEnvironmentalDataCorrected)] <- 0 #Replace NAs with 0s
head(dailyEnvironmentalDataCorrected) #Confirm changes
```

```{r}
envNMDSData <- data.trans(dailyEnvironmentalDataCorrected, method = 'hellinger', plot = FALSE) #Hellinger (asymmetric) transformation
```

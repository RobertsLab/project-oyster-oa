#In this script, I'll use an NMDS plot to assess technical replication of my PRTC peptides, which is something Emma was interested in. I'll do this with both normalized and nonnormalized data.

#### SET WORKING DIRECTORY ####
setwd(dir = "../..") #Set my working directory as main SRM analysis folder, and not the subdirectory where the R script is located
...
#For the NMDS, I want only the protein/peptide/transition information and peak area for PRTC peptides
...
SRMDataNMDS <- SRMDataNMDS[ SRMDataNMDS$Protein.Name %in% "PRTC peptides", ] #Keep only PRTC peptide data
...
SRMDataNMDSNonNormalizedPivoted$RowNames <- paste(SRMDataNMDSNonNormalizedPivoted$Peptide.Sequence, SRMDataNMDSNonNormalizedPivoted$Fragment.Ion) #Merge Protein, Peptide and Transition information into one column
...
#proc.nmds.nonnorm.euclidean <- metaMDS(area.t, distance = 'bray', k = 2, trymax = 10000, autotransform = FALSE) #Make MDS dissimilarity matrix using euclidean distance. Julian confirmed that I should use euclidean distances, and not bray-curtis
...
#Couldn't use nontransformed data: Error in if (any(w < 0) || sum(w) == 0) stop("weights must be non-negative and not all zero") : missing value where TRUE/FALSE needed
...
#I saved the two iterations where my data was transformed (log or autotransformation)

#jpeg(filename = "2017-10-10-Troubleshooting/2017-10-24-PRTC-NMDS/2017-10-24-NMDS-TechnicalReplication-NonNormalized-LogTransformation.jpeg", width = 1000, height = 1000)
#ordiplot(proc.nmds.nonnorm.euclidean.log, choices = c(1,2), type = "text", display = "sites") #Plot refined NMDS displaying only samples with their names
#dev.off()

#jpeg(filename = "2017-10-10-Troubleshooting/2017-10-24-PRTC-NMDS/2017-10-24-NMDS-TechnicalReplication-NonNormalized-Autotransformation.jpeg", width = 1000, height = 1000)
#ordiplot(proc.nmds.nonnorm.euclidean.autotransform, choices = c(1,2), type = "text", display = "sites") #Plot refined NMDS displaying only samples with their names
#dev.off()
...
#The log transformed and no transformation plots looked similar. The autotransformed plot looked like the previous nonnormalized autotransformed plot. I'll save the log and autotransformed versions for comparison.

#jpeg(filename = "2017-10-10-Troubleshooting/2017-10-24-PRTC-NMDS/2017-10-24-NMDS-TechnicalReplication-Normalized-LogTransformation.jpeg", width = 1000, height = 1000)
#ordiplot(proc.nmds.euclidean.log, choices = c(1,2), type = "text", display = "sites") #Plot refined NMDS displaying only samples with their names
#dev.off()

#jpeg(filename = "2017-10-10-Troubleshooting/2017-10-24-PRTC-NMDS/2017-10-24-NMDS-TechnicalReplication-Normalized-Autotransformation.jpeg", width = 1000, height = 1000)
#ordiplot(proc.nmds.euclidean.autotransform, choices = c(1,2), type = "text", display = "sites") #Plot refined NMDS displaying only samples with their names
#dev.off()

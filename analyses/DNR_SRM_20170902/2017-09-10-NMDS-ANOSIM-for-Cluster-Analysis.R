...
SRMDataNMDSNonNormalizedAveragedCorrected <- SRMDataNMDSNonNormalizedAveraged #Duplicate dataframe
SRMDataNMDSNonNormalizedAveragedCorrected[is.na(SRMDataNMDSNonNormalizedAveragedCorrected)] <- 0 #Replace NAs with 0s
head(SRMDataNMDSNonNormalizedAveragedCorrected) #Confirm there are no NAs

area.protID3 <- SRMDataNMDSNonNormalizedAveragedCorrected[,-49] #Save all area data as a new dataframe except for OBLNK2
...
proc.nmds.nonnorm.averaged.euclidean <- metaMDS(area3.t, distance = 'euclidean', k = 2, trymax = 10000, autotransform = FALSE) #Make MDS dissimilarity matrix using euclidean distance. Julian confirmed that I should use euclidean distances, and not bray-curtis
stressplot(proc.nmds.nonnorm.averaged.euclidean) #Make Shepard plot
vec.proc.nmds.nonnorm.averaged.euclidean <- envfit(proc.nmds.nonnorm.averaged.euclidean$points, area4.t, perm = 1000) #Calculate loadings
ordiplot(proc.nmds.nonnorm.averaged.euclidean, choices = c(1,2), type = "points", display = "sites") #Plot basic NMDS
plot(vec.proc.nmds.nonnorm.averaged.euclidean, p.max=.01, col='blue') #Plot eigenvectors
...
jpeg(filename = "2017-09-11-NMDS-NonNorm-Analysis-Averaged", width = 1000, height = 1000)
fig.nmds <- ordiplot(proc.nmds.nonnorm.averaged.euclidean, choices=c(1,2), type='none', display='sites', xlab='Axis 1', ylab='Axis 2', cex=0.5) #Save NMDS as a new object

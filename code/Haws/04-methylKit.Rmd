...
```{r}
nFiles <- length(analysisFiles) #Count number of samples
...
Looking at the PCA, samples 3H-2 (14) and 2H-3 (3) are likely outliers, with samples 2H-1 (1) and 3H-3 (15) also looking like potential outliers. Samples 3 and 14 had 36-39 M sequences, which were the two lowest number of sequences out of all the samples. There were no sequencing or trimming criteria that flagged me to remove samples 1 and 15.
...
I will perform a beta-dispersion test with the methylation data to determine if there are differences in dispersion between ploidy conditions and pH treatments. If there are dispersion differences, I will need to apply an overdispersion correction to the data when identifying DML.
...
```{r}
dissim.meth <- vegdist(scale(methInfoDisp[c(5:5953594)]), "euclidean", na.rm = TRUE) #Create euclidean dissimilarity matrix to use for PERMDISP, removing rows with NA values.
```
...
```{r}
rbind(broom::tidy(anova(disp.meth.ploidy)) %>%
        mutate(var = "ploidy"),
      broom::tidy(anova(disp.meth.pH)) %>%
        mutate(var = "pH")) %>%
  write.csv("PERMDISP-results.csv", quote = FALSE, row.names = FALSE) #Combine PERMDISP output for each variable and add specifics for which variable was tested. Save output
```
...
I will identify DML with and without covariates to determine if adding the covariate impacts DML identification.

#### With covariates

```{r}
differentialMethylationStatsPloidyCovar <- methylKit::calculateDiffMeth(methylationInformationFilteredCov5,
...
head(differentialMethylationStatsPloidyCovar) #Look at differential methylation statistics
```

```{r}
diffMethStatsPloidyCovar25 <- methylKit::getMethylDiff(differentialMethylationStatsPloidyCovar, difference = 25, qvalue = 0.01) #Identify loci that are at least 50% different
length(diffMethStatsPloidyCovar25$chr) #Count the number of DML
head(diffMethStatsPloidyCovar25) #Confirm creation
```

```{r}
diffMethStatsPloidyCovar50 <- methylKit::getMethylDiff(differentialMethylationStatsPloidyCovar, difference = 50, qvalue = 0.01) #Identify loci that are at least 25% different
length(diffMethStatsPloidyCovar50$chr) #Count the number of DML
head(diffMethStatsPloidyCovar50) #Confirm creation
```

```{r}
diffMethStatsPloidyCovar75 <- methylKit::getMethylDiff(differentialMethylationStatsPloidyCovar, difference = 75, qvalue = 0.01) #Identify loci that are at least 75% different
length(diffMethStatsPloidyCovar75$chr) #Count the number of DML
head(diffMethStatsPloidyCovar75) #Confirm creation
```
...
```{r}
write.csv(diffMethStatsPloidyCovar25, "DML/DML-ploidy-25-Covar-Cov5.csv", quote = FALSE) #Save table as .csv
write.csv(diffMethStatsPloidyCovar50, "DML/DML-ploidy-50-Covar-Cov5.csv", quote = FALSE) #Save table as .csv
write.csv(diffMethStatsPloidyCovar75, "DML/DML-ploidy-75-Covar-Cov5.csv", quote = FALSE) #Save table as .csv
```
...
differentialMethylationStatsPloidy <- methylKit::calculateDiffMeth(methylationInformationFilteredCov5,
                                                                   mc.cores = 40) #Calculate differential methylation statistics based on treatment indication from methRead. Use 40 cores. Do not correct for overdispersion.
head(differentialMethylationStatsPloidy) #Look at differential methylation statistics
```

```{r}
diffMethStatsPloidy25 <- methylKit::getMethylDiff(differentialMethylationStatsPloidy, difference = 25, qvalue = 0.01) #Identify loci that are at least 25% different
...
```{r}
methylationInformationFilteredCov5pH <- methylKit::reorganize(methylationInformationFilteredCov5,
                                                             sample.id = sampleMetadata$sampleID,
                                                             treatment = sampleMetadata$pHTreatment) #Reorganize methylationInformationFilteredCov5 to provide pH treatment specification. This is possible because loci with 5x coverage in all samples were retained. Modifying min.per.group would mean that another unite would have to be done.
head(methylationInformationFilteredCov5pH) #Confirm unite
```

### Identify DML

```{r}
differentialMethylationStatspHCovar <- methylKit::calculateDiffMeth(methylationInformationFilteredCov5pH,
...
head(differentialMethylationStatspHCovar) #Look at differential methylation statistics
```

```{r}
diffMethStatspHCovar25 <- methylKit::getMethylDiff(differentialMethylationStatspHCovar, difference = 25, qvalue = 0.01) #Identify loci that are at least 75% different
length(differentialMethylationStatspHCovar$chr) #Count the number of DML
head(differentialMethylationStatspHCovar) #Confirm creation
```

```{r}
diffMethStatspHCovar50 <- methylKit::getMethylDiff(differentialMethylationStatspHCovar, difference = 50, qvalue = 0.01) #Identify loci that are at least 50% different
length(diffMethStatspHCovar50$chr) #Count the number of DML
head(diffMethStatspHCovar50) #Confirm creation
```

```{r}
diffMethStatspHCovar75 <- methylKit::getMethylDiff(differentialMethylationStatspHCovar, difference = 75, qvalue = 0.01) #Identify loci that are at least 75% different
length(diffMethStatspHCovar75$chr) #Count the number of DML
head(diffMethStatspHCovar75) #Confirm creation
```
...
```{r}
write.csv(diffMethStatspHCovar25, "DML/DML-pH-25-Covar-Cov5.csv", quote = FALSE) #Save table as .csv
write.csv(diffMethStatspHCovar50, "DML/DML-pH-50-Covar-Cov5.csv", quote = FALSE) #Save table as .csv
write.csv(diffMethStatspHCovar75, "DML/DML-pH-75-Covar-Cov5.csv", quote = FALSE) #Save table as .csv
```

...
```{r}
dissim.meth <- vegdist(scale(methylationInformationFilteredCov5[c(5:601)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```

```{r}
permanova.meth <- adonis2(dissim.meth ~ as.factor(ploidy)*as.factor(pH), data = methylationInformationFilteredCov5, method = 'eu', by = "terms") #Conduct global PERMANOVA to assess significance of trends in PCA. Scale metabolomics data and assess influence of ploidy, pH, and their interaction on methylation. Include "by = "terms"" to ensure test assesses significance for each term.
permanova.meth #Significant influence of all factors
```

```{r}
broom::tidy(permanova.meth) %>%
  write.csv(., "PERMANOVA-results.csv", quote = FALSE) #Save PERMANOVA output
```

```{r}
disp.meth.ploidy <- betadisper(dissim.meth, group = as.factor(methylationInformationFilteredCov5$treatment), type = 'centroid') #Run a beta dispersion model to assess if significant differences are due to differences in group centroid or variance
plot(disp.meth.ploidy) #Dispersion differences exist
boxplot(disp.meth.ploidy) #Show distance to group centroids, can clearly see dispersion differences between 30 and the other two
anova(disp.meth.ploidy) #Variance is different between groups. Temperature significance is due to centroid and dispersion differences!
```

```{r}
disp.meth.pH <- betadisper(dissim.meth, group = as.factor(methylationInformationFilteredCov5$day), type = 'centroid') #Run a beta dispersion model to assess if significant differences are due to differences in group centroid or variance
plot(disp.meth.pH) #Similar dispersion, but some clear outliers for day 22
boxplot(disp.meth.pH) #Similar dispersion differences, outliers seen more easily
anova(disp.meth.pH) #Variance is the same between groups. pH significance is due to centroid differences!
```

```{r}
disp.meth.ploidypH <- betadisper(dissim.meth, group = as.factor(methylationInformationFilteredCov5$treatment_pH), type = 'centroid') #Run a beta dispersion model to assess if significant differences are due to differences in group centroid or variance
plot(disp.meth.ploidypH) #Dispersion differences exist, especially with 30_22
boxplot(disp.meth.ploidypH) #Very clear dispersion differences for each unique treatment!
anova(disp.meth.ploidypH) #Variance is different between groups. Interaction significance is due to centroid and dispersion differences!
```
...
rbind(broom::tidy(anova(disp.meth.ploidy)) %>%
      ...
      broom::tidy(anova(disp.meth.pH)) %>%
        mutate(var = "pH"),
      broom::tidy(anova(disp.meth.ploidypH)) %>%

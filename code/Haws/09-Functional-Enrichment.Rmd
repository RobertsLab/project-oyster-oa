...
resultFisherBP <- getSigGroups(ploidyGOdataBP, test.stat)
resultFisherBP
```
...
```{r}
resultFisherCC <- getSigGroups(ploidyGOdataCC, test.stat)
resultFisherCC
```
...
```{r}
resultFisherMF <- getSigGroups(ploidyGOdataMF, test.stat) #Extract significant GOterms using Fisher exact test
resultFisherMF #Get results summary
```

```{r}
pvalFis.ploidyMF <- score(resultFisherMF) #Extract p-values
...
```{r}
allRes.ploidyMF <- GenTable(ploidyGOdataMF, classic = resultFisherMF, ranksOf = "classic", orderBy = "classic", topNodes = length(pvalFis.ploidyMF)) #Create a statistical results table with statistical test results. Order by p-value (classic), and include all results (topNodes)
